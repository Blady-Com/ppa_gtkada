...
package body Window1_Callbacks  is
   ...
   begin
      if Gtk_Check_Button (Get_Widget (Builder, "checkbutton1")).Get_Active then
         Set_Text
           (Gtk_Label (Get_Widget (Builder, "label2")),
            Long_Long_Integer'Image (Factorielle (Natural'Value (S))));
      else
         Set_Text (Gtk_Label (Get_Widget (Builder, "label2")), Factorielle (Natural'Value (S)));
      end if;
   exception
      when others =>
         Set_Text (Gtk_Label (Get_Widget (Builder, "label2")), "Erreur!");
